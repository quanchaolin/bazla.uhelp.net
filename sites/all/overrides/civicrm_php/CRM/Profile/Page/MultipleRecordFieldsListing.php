...
class CRM_Profile_Page_MultipleRecordFieldsListing extends CRM_Core_Page_Basic {
  ...
  public function browse() {
    ...
    elseif ($this->_pageViewType == 'customDataView') {
      ...
      $hide_links = False;
      if ($groupDetail[$customGroupId]['name'] === 'Employment_History') {
        foreach ($groupDetail[$customGroupId]['fields'] as $pkey => $pfieldID) {
          if ($groupDetail[$customGroupId]['fields'][$pkey]['column_name'] === 'salary_yearly__90') {
            // only show salary field to correct ACL - temp one for now //View Salaries
            if (!CRM_Core_Permission::check('view salary')) {
              unset($groupDetail[$customGroupId]['fields'][$pkey]);
              $hide_links = True;
            }
